...
@pytest.mark.parametrize(**locators.RegisterFormLocators.DATA_FOR_CORRECT_REGISTER)
...
@pytest.mark.parametrize(**locators.LoginFormLocators.DATA_FOR_INCORRECT_LOGIN)
...
@pytest.mark.parametrize(**locators.LoginFormLocators.DATA_FOR_INCORRECT_LOGIN)
def test_incorrect_register_data(browser, username, password, comment):
    page = LoginForm(browser, locators.URL)
    page.open()
    page.go_to_register_form()
    register_form = RegisterForm(browser, browser.current_url)
    register_form.input_data_for_register_form(username, password)
    register_form.check_for_incorrect_register(comment)
